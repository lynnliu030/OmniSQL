...
def openai_generate_multi_turn(model_name, local_port, chat_prompts, db_files, sampling_params, model_type: Literal["local", "openai"],num_turns=5, format="rl"):
    if model_type == "local":
        # local host model
        client = OpenAI(
    ...
    with ThreadPoolExecutor(max_workers=os.cpu_count()) as executor:
        for turn in range(num_turns):
                ...
                for future in tqdm(as_completed(futures), total=len(futures), desc=f"Turn {turn+1}"):
                    ...
                    # hack, fix later
                    obs_str = convert_observation_to_string(observation, turn, num_turns)
        ...
        for future in tqdm(as_completed(futures), total=len(futures), desc=f"Final Turn"):
            idx, response = future.result()
            text = response.choices[0].message.content
            patched_text = patch_openai_stop_string(text)
            outputs[idx] += patched_text
            chat_prompts[idx].append({"role": "assistant", "content": patched_text})
    return outputs, chat_prompts
...
if __name__ == '__main__':
    ...
    assert opt.dataset != "bird", f"OPT dataset is bird, completely wrong"
    print(opt)
    ...
    if opt.infer:
        ...
        if opt.model_type == "local": 
            ...
            num_turns=opt.num_turns, format=opt.prompt_format
        )
    ...
    if opt.eval: 
        ...
        if opt.dataset == "spider": # for "spider"
            ...
            raise NotImplementedError("Not implemented for the other datasets")
