...
import random
import re
from evaluate_helper import major_voting, mark_invalid_sqls
import tempfile
import subprocess
...
def load_json_or_jsonl(file_path, as_test: bool = False):
    try:
...
def run_spider_eval(gold_file, pred_file, db_path, ts_db_path, mode, save_pred_sqls, as_test: bool = False):
    ...
    if as_test: 
        gold_sqls = gold_sqls[:10]
        db_ids = db_ids[:10]
    pred = load_json_or_jsonl(pred_file, as_test)
    ...
    cmd = f'python3 -u ~/lshu/BIRD/OmniSQL/train_and_evaluate/test_suite_sql_eval/evaluation.py --gold {gold_file} --pred {temp_file_name} --db {db_path} --etype exec'
    print(cmd)
    ...
    if ts_db_path != "":
        ...
        cmd = f'python3 -u ~/lshu/BIRD/OmniSQL/train_and_evaluate/test_suite_sql_eval/evaluation.py --gold {gold_file} --pred {temp_file_name} --db {ts_db_path} --etype exec'
        result = subprocess.run(cmd, shell=True, text=True, capture_output=True)
        stdout = result.stdout
        print(result.stderr)
        # match the last num in the string
